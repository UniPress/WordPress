...
class WPListTable
{
    ...
    function set_pagination_args($args)
    {
        ...
        if (!headers_sent() && (!defined(
                    'DOING_AJAX'
                ) || !DOING_AJAX) && $args['total_pages'] > 0 && $this->get_pagenum() > $args['total_pages']
        ) {
    ...
    function view_switcher($current_mode)
    {
        ...
        <div class="view-switch">
            ...
            foreach ($modes as $mode => $title) {
                ...
                echo "<a href='" . esc_url(
                        add_query_arg('mode', $mode, $_SERVER['REQUEST_URI'])
                    ) . "' $class><img id='view-switch-$mode' src='" . esc_url(
    ...
    function comments_bubble($post_id, $pending_comments)
    {
        ...
        echo "<a href='" . esc_url(
                add_query_arg('p', $post_id, admin_url('edit-comments.php'))
            ) . "' title='" . esc_attr(
                $pending_phrase
            ) . "' class='post-com-count'><span class='comment-count'>" . number_format_i18n(
                get_comments_number()
            ) . "</span></a>";
