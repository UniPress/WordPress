 ...
 * @access private
 */
class WPMSUsersListTable extends WPListTable
{
                ...
                foreach ($columns as $column_name => $column_display_name) :
                    ...
                    switch ($column_name) {
                        case 'cb':
                            ...
                            <th scope="row" class="check-column">
                                <label class="screen-reader-text"
                                       for="blog_<?php echo $user->ID; ?>"><?php echo sprintf(
                                        __('Select %s'),
                                        $user->user_login
                                    ); ?></label>
                        ...
                        case 'username':
                            ...
                            if (current_user_can('delete_user', $user->ID) && !in_array(
                                    $user->user_login,
                                    $super_admins
                                )
                            ) {
                                $actions['delete'] = '<a href="' . $delete = esc_url(
                                            network_admin_url(
                                                add_query_arg(
                                                    '_wp_http_referer',
                                                    urlencode(wp_unslash($_SERVER['REQUEST_URI'])),
                        ...
                        case 'blogs':
                            ...
                            if (is_array($blogs)) {
                                foreach ((array)$blogs as $key => $val) {
                                    ...
                                    echo '<a href="' . esc_url(
                                            network_admin_url('site-info.php?id=' . $val->userblog_id)
                                        ) . '">' . str_replace(
                                    ...
                                    $actions['view'] = '<a class="' . $class . '" href="' . esc_url(
                                            get_home_url($val->userblog_id)
                                        ) . '">' . __('View') . '</a>';
