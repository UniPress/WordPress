 ...
 * @access private
 */
class WPThemeInstallListTable extends WPThemesListTable
{
    ...
    function prepare_items()
    {
        ...
        if (is_wp_error($api)) {
            wp_die(
                ...
                ) . '</p> <p><a href="#" onclick="document.location.reload(); return false;">' . __(
                    'Try again'
                ) . '</a>'
            );
        }
    ...
    function single_row($theme)
    {
        ...
        switch ($status) {
            default:
            case 'install':
                $actions[] = '<a class="install-now" href="' . esc_url(
            ...
            case 'update_available':
                $actions[] = '<a class="install-now" href="' . esc_url(
                    ...
                    ) . '" title="' . esc_attr(sprintf(__('Update to version %s'), $theme->version)) . '">' . __(
                        'Update'
                    ) . '</a>';
                break;
            case 'newer_installed':
            case 'latest_installed':
                $actions[] = '<span class="install-now" title="' . esc_attr__(
                        'This theme is already installed and is up to date'
                    ) . '">' . _x('Installed', 'theme') . '</span>';
                break;
        }

        $actions[] = '<a class="install-theme-preview" href="' . esc_url($preview_url) . '" title="' . esc_attr(
            ...
            <div class="wp-full-overlay-sidebar">
                ...
                <div class="wp-full-overlay-footer">
                    <a href="#" class="collapse-sidebar button-secondary"
                       title="<?php esc_attr_e('Collapse Sidebar'); ?>">
    ...
    function install_theme_info($theme)
    {
        ...
        $num_ratings = sprintf(
            _n('(based on %s rating)', '(based on %s ratings)', $theme->num_ratings),
            number_format_i18n($theme->num_ratings)
        );
        ...
        <div class="install-theme-info"><?php
            switch ($status) {
                default:
                case 'install':
                    echo '<a class="theme-install button-primary" href="' . esc_url(
                            wp_nonce_url($install_url, 'install-theme_' . $theme->slug)
                        ) . '">' . __('Install') . '</a>';
                    break;
                case 'update_available':
                ...
                case 'newer_installed':
                case 'latest_installed':
                    echo '<span class="theme-install" title="' . esc_attr__(
                            'This theme is already installed and is up to date'
                        ) . '">' . _x('Installed', 'theme') . '</span>';
                    break;
            } ?>
            ...
            <div class="theme-details">
                <div class="star-holder" title="<?php echo esc_attr($num_ratings); ?>">
                    <div class="star-rating"
                         style="width:<?php echo esc_attr(intval($theme->rating) . 'px'); ?>;"></div>
                </div>
