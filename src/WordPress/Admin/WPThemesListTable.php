 ...
 * @access private
 */
class WPThemesListTable extends WPListTable
{
    ...
    function prepare_items()
    {
        ...
        if (!empty($_REQUEST['s'])) {
            $this->search_terms = array_unique(
                array_filter(array_map('trim', explode(',', strtolower(wp_unslash($_REQUEST['s'])))))
            );
        }
        ...
        foreach ($themes as $theme):
            ...
            <div class="available-theme"><?php
                ...
                $activate_link = wp_nonce_url(
                    "themes.php?action=activate&amp;template=" . urlencode($template) . "&amp;stylesheet=" . urlencode(
                        $stylesheet
                    ),
                ...
                if (current_user_can('edit_theme_options')) {
                    $actions['preview'] .= '<a href="' . wp_customize_url(
                            $stylesheet
