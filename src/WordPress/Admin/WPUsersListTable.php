 ...
 * @access private
 */
class WPUsersListTable extends WPListTable
{
    ...
    function get_views()
    {
        ...
        foreach ($wp_roles->get_names() as $this_role => $name) {
            ...
            $role_links[$this_role] = "<a href='" . esc_url(
                    add_query_arg('role', $this_role, $url)
                ) . "'$class>$name</a>";
        }
    ...
    function single_row($user_object, $style = '', $role = '', $numposts = 0)
    {
        ...
        if (!(is_object($user_object) && is_a($user_object, 'WPUser')))
        ...
        if (current_user_can('list_users')) {
            ...
            $checkbox = '<label class="screen-reader-text" for="cb-select-' . $user_object->ID . '">' . sprintf(
                    __('Select %s'),
        ...
        foreach ($columns as $column_name => $column_display_name) {
            ...
            switch ($column_name) {
                ...
                case 'name':
                    ...
                    break;
                case 'email':
                    $r .= "<td $attributes><a href='mailto:$email' title='" . esc_attr(
                            sprintf(__('E-mail: %s'), $email)
                        ) . "'>$email</a></td>";
                    break;
                case 'role':
                ...
                case 'posts':
                    ...
                    $r .= "<td $attributes>";
                    if ($numposts > 0) {
                        $r .= "<a href='edit.php?author=$user_object->ID' title='" . esc_attr__(
