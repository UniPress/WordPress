...
class WPHttp {
	 ...
	 * @return array|object Array containing 'headers', 'body', 'response', 'cookies', 'filename'. A WPError instance upon error
	 */
	function request( $url, $args = array() ) {
		...
		if ( empty( $url ) || empty( $arrURL['scheme'] ) )
			return new WordPress\WPError('http_request_failed', __('A valid URL was not provided.'));

		if ( $this->block_request( $url ) )
			return new WordPress\WPError( 'http_request_failed', __( 'User has blocked requests through HTTP.' ) );
		...
		if ( $r['stream'] ) {
			...
			if ( ! wp_is_writable( dirname( $r['filename'] ) ) )
				return new WordPress\WPError( 'http_request_failed', __( 'Destination directory for file streaming does not exist or is not writable.' ) );
		}
	...
	private function _dispatch_request( $url, $args ) {
		...
		if ( !$class )
			return new WordPress\WPError( 'http_failure', __( 'There are no HTTP transports available which can complete the requested request.' ) );
	 ...
	 *
	 * If it's found, then it upgrades any basic name => value pairs to WPHttpCookie instances,
	 ...
	 * @return false|object False if no redirect is present, a WP_HTTP or WPError result otherwise.
	 */
