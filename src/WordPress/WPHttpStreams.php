...
class WPHttpStreams {
    ...
    function request($url, $args = array()) {
        ...
        if ( false === $handle ) {
            ...
            if ( $secure_transport && 0 === $connection_error && '' === $connection_error_str )
                return new WordPress\WPError( 'http_request_failed', __( 'The SSL certificate for the host could not be verified.' ) );

            return new WordPress\WPError('http_request_failed', $connection_error . ': ' . $connection_error_str );
        }
            ...
            while ( ! feof($handle) && $keep_reading ) {
                ...
                if ( ! $bodyStarted ) {
                    ...
                    if ( strpos( $strResponse, "\r\n\r\n" ) ) {
                        $process = WPHttp::processResponse( $strResponse );
                        $bodyStarted = true;
                        $block = $process['body'];
                        unset( $strResponse );
                        $process['body'] = '';
        ...
        $arrHeaders = WPHttp::processHeaders( $process['headers'], $url );
        ...
        if ( ! empty( $process['body'] ) && isset( $arrHeaders['headers']['transfer-encoding'] ) && 'chunked' == $arrHeaders['headers']['transfer-encoding'] )
            $process['body'] = WPHttp::chunkTransferDecode($process['body']);
    ...
    static function verify_ssl_certificate( $stream, $host ) {
        ...
        $host_type = ( WPHttp::is_ip_address( $host ) ? 'ip' : 'dns' );
