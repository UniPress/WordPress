...
final class WPTheme implements \ArrayAccess {
	...
	/**
	 * Constructor for WPTheme.
	 *
	 * @param string $theme_dir Directory of the theme within the theme_root.
	 * @param string $theme_root Theme root.
	 * @param WPError|null $_child If this theme is a parent theme, the child may be passed for validation purposes.
	 */
	public function __construct( $theme_dir, $theme_root, $_child = null ) {
		...
		if ( ! isset( self::$persistently_cache ) ) {
			self::$persistently_cache = apply_filters( 'wp_cache_themes_persistently', false, 'WPTheme' );
		...
		if ( is_array( $cache ) ) {
			...
			if ( ! file_exists( $this->theme_root . '/' . $this->stylesheet ) )
				$this->errors = new \WordPress\WP_Error( 'theme_not_found', sprintf( __( 'The theme directory "%s" does not exist.' ), $this->stylesheet ) );
			else
				$this->errors = new \WordPress\WP_Error( 'theme_no_stylesheet', __( 'Stylesheet is missing.' ) );
			...
			$this->headers['Name'] = $this->stylesheet;
			$this->errors = new \WordPress\WP_Error( 'theme_stylesheet_not_readable', __( 'Stylesheet is not readable.' ) );
			$this->template = $this->stylesheet;
			$this->cache_add( 'theme', array( 'headers' => $this->headers, 'errors' => $this->errors, 'stylesheet' => $this->stylesheet, 'template' => $this->template ) );
			return;
		} else {
		...
		if ( ! $this->template && ! ( $this->template = $this->headers['Template'] ) ) {
			...
			if ( ! file_exists( $this->theme_root . '/' . $this->stylesheet . '/index.php' ) ) {
				$this->errors = new \WordPress\WP_Error( 'theme_no_index', __( 'Template is missing.' ) );
				$this->cache_add( 'theme', array( 'headers' => $this->headers, 'errors' => $this->errors, 'stylesheet' => $this->stylesheet, 'template' => $this->template ) );
				return;
			}
		}
		...
		if ( ! is_array( $cache ) && $this->template != $this->stylesheet && ! file_exists( $this->theme_root . '/' . $this->template . '/index.php' ) ) {
			...
			if ( '.' != $parent_dir && file_exists( $this->theme_root . '/' . $parent_dir . '/' . $this->template . '/index.php' ) ) {
				...
				$this->errors = new \WordPress\WP_Error( 'theme_no_parent', sprintf( __( 'The parent theme is missing. Please install the "%s" parent theme.' ), $this->template ) );
				$this->cache_add( 'theme', array( 'headers' => $this->headers, 'errors' => $this->errors, 'stylesheet' => $this->stylesheet, 'template' => $this->template ) );
				$this->parent = new \WordPress\WPTheme( $this->template, $this->theme_root, $this );
				return;
			}
		}
		...
		if ( $this->template != $this->stylesheet ) {
			...
			if ( is_a( $_child, 'WPTheme' ) && $_child->template == $this->stylesheet ) {
				$_child->parent = null;
				$_child->errors = new \WordPress\WP_Error( 'theme_parent_invalid', sprintf( __( 'The "%s" theme is not a valid parent theme.' ), $_child->template ) );
				$_child->cache_add( 'theme', array( 'headers' => $_child->headers, 'errors' => $_child->errors, 'stylesheet' => $_child->stylesheet, 'template' => $_child->template ) );
				// The two themes actually reference each other with the Template header.
				if ( $_child->stylesheet == $this->template ) {
					$this->errors = new \WordPress\WP_Error( 'theme_parent_invalid', sprintf( __( 'The "%s" theme is not a valid parent theme.' ), $this->template ) );
					$this->cache_add( 'theme', array( 'headers' => $this->headers, 'errors' => $this->errors, 'stylesheet' => $this->stylesheet, 'template' => $this->template ) );
				}
				return;
			}
			// Set the parent. Pass the current instance so we can do the crazy checks above and assess errors.
			$this->parent = new \WordPress\WPTheme( $this->template, isset( $theme_root_template ) ? $theme_root_template : $this->theme_root, $this );
		}
	 ...
	 * @access public
	 *
	 * @return WPError|bool WPError if there are errors, or false.
	 */
	 ...
	 * @access public
	 *
	 * @return WPTheme|bool Parent theme, or false if the current theme is not a child theme.
	 */
	 ...
	 * for all other URLs returned by WPTheme, so we pass it to the public function
	...
	public static function sort_by_name( &$themes ) {
		if ( 0 === strpos( get_locale(), 'en_' ) ) {
			uasort( $themes, array( '\\WordPress\\WPTheme', '_name_sort' ) );
		} else {
			uasort( $themes, array( '\\WordPress\\WPTheme', '_name_sort_i18n' ) );
		}
	}
