 ...
 * @return mixed WPError on failure, string Filename on success.
 */
function download_url( $url, $timeout = 300 ) {
	//WARNING: The file is not automatically deleted, The script must unlink() the file.
	if ( ! $url )
		return new WordPress\WPError('http_no_url', __('Invalid URL Provided.'));

	$tmpfname = wp_tempnam($url);
	if ( ! $tmpfname )
		return new WordPress\WPError('http_no_file', __('Could not create Temporary file.'));
	...
	if ( 200 != wp_remote_retrieve_response_code( $response ) ){
		unlink( $tmpfname );
		return new WordPress\WPError( 'http_404', trim( wp_remote_retrieve_response_message( $response ) ) );
	}
 ...
 * @return bool|object WPError on failure, true on success, false when the MD5 format is unknown/unexpected
 */
function verify_file_md5( $filename, $expected_md5 ) {
	...
	return new WordPress\WPError( 'md5_mismatch', sprintf( __( 'The checksum of the file (%1$s) does not match the expected checksum value (%2$s).' ), bin2hex( $file_md5 ), bin2hex( $expected_raw_md5 ) ) );
}
...
function _unzip_file_ziparchive($file, $to, $needed_dirs = array() ) {
	...
	if ( true !== $zopen )
		return new WordPress\WPError( 'incompatible_archive', __( 'Incompatible Archive.' ), array( 'ziparchive_error' => $zopen ) );

	$uncompressed_size = 0;

	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		if ( ! $info = $z->statIndex($i) )
			return new WordPress\WPError( 'stat_failed_ziparchive', __( 'Could not retrieve file from archive.' ) );
	...
	if ( defined( 'DOING_CRON' ) && DOING_CRON ) {
		...
		if ( $available_space && ( $uncompressed_size * 2.1 ) > $available_space )
			return new WordPress\WPError( 'disk_full_unzip_file', __( 'Could not copy files. You may have run out of disk space.' ), compact( 'uncompressed_size', 'available_space' ) );
	}
	...
	foreach ( $needed_dirs as $_dir ) {
		if ( ! $wp_filesystem->mkdir($_dir, FS_CHMOD_DIR) && ! $wp_filesystem->is_dir($_dir) ) // Only check to see if the Dir exists upon creation failure. Less I/O this way.
			return new WordPress\WPError( 'mkdir_failed_ziparchive', __( 'Could not create directory.' ), substr( $_dir, strlen( $to ) ) );
	}
	unset($needed_dirs);
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		...
		if ( false === $contents )
			return new WordPress\WPError( 'extract_failed_ziparchive', __( 'Could not extract file from archive.' ), $info['name'] );

		if ( ! $wp_filesystem->put_contents( $to . $info['name'], $contents, FS_CHMOD_FILE) )
			return new WordPress\WPError( 'copy_failed_ziparchive', __( 'Could not copy file.' ), $info['name'] );
	}
...
function _unzip_file_pclzip($file, $to, $needed_dirs = array()) {
	...
	if ( !is_array($archive_files) )
		return new WordPress\WPError('incompatible_archive', __('Incompatible Archive.'), $archive->errorInfo(true));

	if ( 0 == count($archive_files) )
		return new WordPress\WPError( 'empty_archive_pclzip', __( 'Empty archive.' ) );
	...
	foreach ( $needed_dirs as $_dir ) {
		...
		if ( ! $wp_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $wp_filesystem->is_dir( $_dir ) )
			return new WordPress\WPError( 'mkdir_failed_pclzip', __( 'Could not create directory.' ), substr( $_dir, strlen( $to ) ) );
	}
	unset($needed_dirs);
	...
	foreach ( $archive_files as $file ) {
		...
		if ( ! $wp_filesystem->put_contents( $to . $file['filename'], $file['content'], FS_CHMOD_FILE) )
			return new WordPress\WPError( 'copy_failed_pclzip', __( 'Could not copy file.' ), $file['filename'] );
	}
	return true;
}
	...
	foreach ( (array) $dirlist as $filename => $fileinfo ) {
		...
		if ( 'f' == $fileinfo['type'] ) {
			if ( ! $wp_filesystem->copy($from . $filename, $to . $filename, true, FS_CHMOD_FILE) ) {
				...
				if ( ! $wp_filesystem->copy($from . $filename, $to . $filename, true, FS_CHMOD_FILE) )
					return new WordPress\WPError( 'copy_failed_copy_dir', __( 'Could not copy file.' ), $to . $filename );
			}
		} elseif ( 'd' == $fileinfo['type'] ) {
			if ( !$wp_filesystem->is_dir($to . $filename) ) {
				if ( !$wp_filesystem->mkdir($to . $filename, FS_CHMOD_DIR) )
					return new WordPress\WPError( 'mkdir_failed_copy_dir', __( 'Could not create directory.' ), $to . $filename );
			}
