...
function delete_theme($stylesheet, $redirect = '') {
	...
	if ( empty($themes_dir) )
		return new WordPress\WPError('fs_no_themes_dir', __('Unable to locate WordPress theme directory.'));
	...
	if ( ! $deleted )
		return new WordPress\WPError('could_not_remove_theme', sprintf(__('Could not fully remove the theme %s.'), $stylesheet) );
...
function themes_api( $action, $args = null ) {
	...
	if ( ! $res ) {
		...
		if ( is_wp_error($request) ) {
			$res = new WordPress\WPError('themes_api_failed', __( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="http://wordpress.org/support/">support forums</a>.' ), $request->get_error_message() );
		} else {
			$res = maybe_unserialize( wp_remote_retrieve_body( $request ) );
			if ( ! is_object( $res ) && ! is_array( $res ) )
				$res = new WordPress\WPError('themes_api_failed', __( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="http://wordpress.org/support/">support forums</a>.' ), wp_remote_retrieve_body( $request ) );
		}
	}
 ...
 * @since 3.8.0
 *
 * @param array $themes Optional. Array of WPTheme objects to prepare.
